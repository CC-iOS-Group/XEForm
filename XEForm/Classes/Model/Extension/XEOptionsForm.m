...
-(instancetype)initWithRow:(XEFormRowObject *)row
{
    ...
    if (self)
    {
        ...
        if(row.placeholder)
        {
            XEFormRowObject *placeholderRow = [[XEFormRowObject alloc] initWithKey:@"0" Class:nil type:XEFormRowTypeBoolean];
        ...
        for (NSUInteger i = 0; i < row.options.count; i++)
        {
            ...
            XEFormRowObject *placeholderRow = [[XEFormRowObject alloc] initWithKey:[@(index) description] Class:nil type:XEFormRowTypeBoolean];
            placeholderRow.action = action;
            placeholderRow.valueTransformer = row.valueTransformer;
